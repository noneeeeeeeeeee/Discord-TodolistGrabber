...
class Status(commands.Cog):
    ...
    async def apistatus(self, ctx):
        ...
        try:
            ...
            if api_data:
                ...
                embed.add_field(
                    name="Response Time", value=f"{responsetime} seconds", inline=False
                )
            ...
            await message.edit(
                content=":x: An error occurred while checking the API status."
            )
            return
    ...
    async def apistatus_error(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            await ctx.send(
                f":hourglass: Please wait {retry_after} seconds before using this command again. This is to prevent spamming to the api"
            )
        else:
