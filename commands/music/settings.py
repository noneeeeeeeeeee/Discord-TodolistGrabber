...
from modules.music.views import DebugView, HelpView, EmbedBuilderView
...
class Settings(commands.Cog, name="settings"):
    ...
    async def playforever(self, ctx: commands.Context):
        ...
        await send(
            ctx,
            "247",
            await get_lang(ctx.guild.id, "enabled" if not toggle else "disabled"),
        )
    ...
    async def view(self, ctx: commands.Context):
        ...
        embed.set_author(
            name=texts[0].format(ctx.guild.name),
    ...
    async def togglecontroller(self, ctx: commands.Context):
        ...
        await send(
            ctx,
            "togglecontroller",
            await get_lang(ctx.guild.id, "enabled" if toggle else "disabled"),
        )
    ...
    async def duplicatetrack(self, ctx: commands.Context):
        ...
        return await send(
            ...
            await get_lang(ctx.guild.id, "disabled" if toggle else "enabled"),
        )
