...
async def check_playlist(
    ctx: commands.Context, name: str = None, full: bool = False, share: bool = True
) -> dict:
    ...
    for index, data in enumerate(user, start=1):
        ...
        if playlist["name"].lower() == name:
            if playlist["type"] == "share" and share:
                playlist = await check_playlist_perms(
                    ctx.author.id, playlist["user"], playlist["referId"]
                )
...
async def search_playlist(
    url: str, requester: discord.Member, time_needed: bool = True
) -> dict:
    try:
        tracks = await voicelink.NodePool.get_node().get_tracks(
...
class Playlists(commands.Cog, name="playlist"):
    ...
    async def playlist_autocomplete(
        ...
        if current:
            return [
                app_commands.Choice(name=p, value=p) for p in playlists if current in p
            ]
        return [app_commands.Choice(name=p, value=p) for p in playlists]

    @commands.hybrid_group(
        name="playlist", aliases=get_aliases("playlist"), invoke_without_command=True
    )
    ...
    async def play(
        self, ctx: commands.Context, name: str = None, value: int = None
    ) -> None:
        ...
        if result["playlist"]["type"] == "link":
            tracks = await search_playlist(
                result["playlist"]["uri"], ctx.author, time_needed=False
            )
        else:
            if not result["playlist"]["tracks"]:
                return await send(
                    ctx, "playlistNoTrack", result["playlist"]["name"], ephemeral=True
                )
    ...
    async def view(self, ctx: commands.Context) -> None:
        ...
        for index, data in enumerate(user, start=1):
            ...
            try:
                if playlist["type"] == "link":
                    tracks = await search_playlist(
                        playlist["uri"], requester=ctx.author
                    )
                    results.append(
                        {
                            "emoji": ("🔒" if max_p < index else "🔗"),
                            "id": data,
                            "time": tracks["time"],
                            "name": playlist["name"],
                            "tracks": tracks["tracks"],
                            "perms": playlist["perms"],
                            "type": playlist["type"],
                        }
                    )
                    ...
                    if share := playlist["type"] == "share":
                        ...
                        if not playlist:
                            await update_user(
                                ctx.author.id, {"$unset": {f"playlist.{data}": 1}}
                            )
                            continue
                    ...
                    results.append(
                        {
                            "emoji": (
                                "🔒" if max_p < index else ("🤝" if share else "❤️")
                            ),
                            "id": data,
                            "time": ctime(time),
                            "name": user[data]["name"],
        ...
        text = await get_lang(
            ctx.guild.id, "playlistViewTitle", "playlistViewHeaders", "playlistFooter"
        )
        embed = discord.Embed(
    ...
    async def create(self, ctx: commands.Context, name: str, link: str = None):
        ...
        await update_user(
            ctx.author.id,
            {"$set": {f"playlist.{assign_playlist_id([data for data in user])}": data}},
        )
    ...
    async def delete(self, ctx: commands.Context, name: str):
        ...
        if result["playlist"]["type"] == "share":
            await update_user(
                result["playlist"]["user"],
                {
                    "$pull": {
                        f"playlist.{result['playlist']['referId']}.perms.read": ctx.author.id
                    }
                },
            )
    ...
    async def share(self, ctx: commands.Context, member: discord.Member, name: str):
        ...
        if result["playlist"]["type"] == "share":
            return await send(
                ctx, "playlistBelongs", result["playlist"]["user"], ephemeral=True
            )
    ...
    @app_commands.describe(
        name="The name of your playlist.", newname="The new name of your playlist."
    )
    @app_commands.autocomplete(name=playlist_autocomplete)
    @commands.dynamic_cooldown(cooldown_check, commands.BucketType.guild)
    ...
    async def inbox(self, ctx: commands.Context) -> None:
        ...
        for data in view.new_playlist[: (max_p - len(user["playlist"]))]:
            ...
            await update_user(
                data["sender"],
                {"$push": {f"playlist.{data['referId']}.perms.read": ctx.author.id}},
            )
    ...
    @commands.dynamic_cooldown(cooldown_check, commands.BucketType.guild)
    @app_commands.describe(
        name="The name of the playlist.", query="Input a query or a searchable link."
    )
    @app_commands.autocomplete(name=playlist_autocomplete)
    async def add(self, ctx: commands.Context, name: str, query: str) -> None:
        ...
        results = await voicelink.NodePool.get_node().get_tracks(
            query, requester=ctx.author
        )
        if not results:
        ...
        await update_user(
            ctx.author.id,
            {"$push": {f'playlist.{result["id"]}.tracks': results[0].track_id}},
        )
        await send(
            ctx,
            "playlistAdded",
    ...
    async def remove(self, ctx: commands.Context, name: str, position: int):
        ...
        await update_user(
            ...
            {
                "$pull": {
                    f'playlist.{result["id"]}.tracks': result["playlist"]["tracks"][
                        position - 1
                    ]
                }
            },
    ...
    async def clear(self, ctx: commands.Context, name: str) -> None:
        ...
        await update_user(
            ctx.author.id, {"$set": {f'playlist.{result["id"]}.tracks': []}}
        )
    ...
    async def export(self, ctx: commands.Context, name: str) -> None:
        ...
        await ctx.send(
            content="",
            file=discord.File(
                StringIO(temp), filename=f"{tracks['name']}_playlist.txt"
            ),
        )
    ...
    async def _import(
        ...
        try:
            ...
            await update_user(
                ...
                {
                    "$set": {
                        f"playlist.{assign_playlist_id([data for data in user])}": data
                    }
                },
            )
            await send(ctx, "playlistCreated", name)
