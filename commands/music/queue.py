...
class MusicQueue(commands.Cog):
    ...
    async def queue(self, ctx):
        ...
        # Format queue with Lavalink tracks
        queue_list = []
        for idx, track in enumerate(player.queue, start=1):
            queue_list.append(f"{idx}. [{track.title}]({track.uri})")
...
class QueuePaginator(discord.ui.View):
    ...
    def create_embed(self):
        ...
        queue_list += "\n".join(
            ...
            for idx, (author, url, ogurl, title, duration) in enumerate(
                page_queue, start=start_idx
            )
        )
