...
class PlayedTrack:
    ...
    title: str
    genres: List[str]
    mood_vector: Optional[Union[List[float], Dict]]  # Can be list or dict with 'vector' key
        ...
        *,
        ...
        if self._verbose >= 1:
            ...
            LOG.info(
                ...
                title, artist, status, progress_ratio * 100, len(self._history)
            )
    ...
    def get_context(self) -> SessionContext:
        ...
        if self._verbose >= 2:
            LOG.debug(
                ...
                focus_genres[:2], skip_rate, consecutive_skips, len(recent_artists)
            )
