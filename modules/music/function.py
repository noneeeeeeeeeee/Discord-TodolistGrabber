...
LOCAL_LANGS: dict[str, dict[str, str]] = (
    {}
...
def get_source(source: str, type: str) -> str:
    source_settings: dict[str, str] = settings.sources_settings.get(
        source.lower().replace(" ", ""), settings.sources_settings.get("others")
    )
        ...
        for key, value in action.items():
            ...
            if mode == "$set":
                ...
                if isinstance(value, dict) and "$each" in value:
                    nested_data.setdefault(cursors[-1], []).extend(
                        value["$each"][value.get("$slice", len(value["$each"])) :]
                    )
                else:
